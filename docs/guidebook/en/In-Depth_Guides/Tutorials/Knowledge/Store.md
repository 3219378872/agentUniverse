...
For a pre-provisioned database, set `create_table: false`. This disables automatic extension, table, and HNSW index creation in both synchronous and asynchronous operations. An administrator must install `vector` and create a table with the expected columns and vector dimensions first. The application role only needs schema usage and the table privileges required by its operations; queries can use a read-only connection. `create_hnsw_index` only applies when `create_table` is enabled. Configuration and embedding validation still apply, and missing resources or insufficient data-access privileges remain errors.
